...
DebSDK.prototype.UpgradeVersion = function (target_dir, version) {
    Shell.pushd(target_dir);
    console.log("Preparing... " + Shell.pwd());
    var c_version = Shell.exec(['dpkg-parsechangelog'].join(' '), {silent:true}).output
        .match(/Version:\s*[0-9_\-\.]+/)[0].replace(/Version:\s*/, '');
    console.log('changelog_v: ' + c_version);
...
DebSDK.prototype.prepare = function(manifest) {
    ...
    this.UpgradeVersion(target_path, version + '-1');
